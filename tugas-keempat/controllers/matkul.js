...
const getMatkul = async (req, res) => {
    try {
        const result = await WithoutParams("SELECT * FROM matkul")
        if (result.length === 0) {
...
const getMatkulById = async (req, res) => {
    try {
        ...
        const result = await WithParams("SELECT * FROM matkul WHERE id = ?", [id])
        if (result.length === 0) {
            return res.status(404).send("Data not found")
        }
        res.status(200).json(result)
...
const postMatkul = async (req, res) => {
    try {
        ...
        const result = await WithParams(
            "INSERT INTO matkul (kd_matkul, nama_matkul, dosen, ruang) VALUES (?, ?, ?, ?)",
...
const updateMatkul = async (req, res) => {
    try {
        ...
        const result = await WithParams(
            "UPDATE matkul SET kd_matkul = ?, nama_matkul = ?, dosen = ?, ruang = ? WHERE id = ?",
            [kd_matkul, nama_matkul, dosen, ruang, id]
        )
...
const deleteMatkul = async (req, res) => {
    try {
        ...
        const result = await WithParams("DELETE FROM matkul WHERE id = ?", [id])
        if(result.affectedRows === 0) {
